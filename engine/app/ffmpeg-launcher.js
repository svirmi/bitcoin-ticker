...
function closeAll() {
  ...
  if (restart) {
    logger.log(
      'We are starting ffmpeg here right after we are closing because we need to restart.'
    );
    restart = false;
...
exports.restart = async function(params) {
  ...
  logger.log(
    'We are restarting ffmpeg, we are noticing a lot of fluctuation on the framerate...'
  );
  restart = true;
